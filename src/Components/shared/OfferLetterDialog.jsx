...
/**
 * Shared Offer Letter Dialog for both Jobs and LIAs
 * @param {Object} props
 * @param {boolean} props.open - Dialog open state
 * @param {Function} props.onClose - Close handler
 * @param {Function} props.onSubmit - Submit handler
 * @param {Object} props.applicant - Applicant data
 * @param {string} props.type - "job" or "lia" - determines which fields to show
 */
export default function OfferLetterDialog({ open, onClose, onSubmit, applicant, type = 'job' }) {
  const accessToken = useSelector((state) => state.auth.accessToken)
  const isLIA = type === 'lia'
  ...
  React.useEffect(() => {
    if (open) {
      setForm({
        ...
        duration: applicant?.offerLetter?.duration || '',
  ...
  const handleSubmit = (event) => {
    ...
    if (isLIA) {
      ...
      payload.compensation = form.compensation
    }
    
    console.log(`Submitting ${isLIA ? 'LIA' : 'Job'} offer with payload:`, payload)
    onSubmit?.(payload)
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => { if (!value) onClose?.() }}>
      <DialogContent className="bg-slate-950 text-white border border-slate-800 max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? 'Edit Offer Letter' : 'Send Offer Letter'}
            {isLIA && <span className="text-sm text-slate-400 ml-2">(LIA Placement)</span>}
          </DialogTitle>
        ...
        <DialogBody>
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            {/* LIAs: Show Duration field */}
            {isLIA && (
              <div className="grid gap-2">
                <Label htmlFor="offer-duration">LIA Duration *</Label>
                <Input
                  id="offer-duration"
                ...
                <p className="text-xs text-slate-500">
                  Specify the duration of the LIA placement
                </p>
              </div>
            )}
